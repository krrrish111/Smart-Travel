...
# Fix the flights empty state text
content = re.sub(r'<p style="color: var\(--text-secondary\); font-size: 1\.1rem;">.*? No hotel bookings found</p>', '<p style="color: var(--text-secondary); font-size: 1.1rem;">✈ No flight bookings found</p>', content)

# Insert HOTELS before TRAINS if missing
hotels_block = """
                <!-- HOTELS -->
...
if 'id="cat-hotels"' not in content:
    content = content.replace('                <!-- TRAINS -->', hotels_block)

with open(filepath, "w", encoding="utf-8") as f:
    f.write(content)

print("Fixed profile.jsp")

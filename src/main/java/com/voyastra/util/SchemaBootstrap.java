...
public class SchemaBootstrap implements ServletContextListener {
    ...
    public void contextInitialized(ServletContextEvent sce) {
        ...
        // 4. Lightweight Database Verification Check
        boolean usersTableExists = false;
        boolean essentialTableExists = false;
        
        try (Connection conn = DBConnection.getConnection();
             Statement stmt = conn.createStatement()) {
            
            // Check 'users' table (verified via SELECT check)
            try (ResultSet rs = stmt.executeQuery("SELECT 1 FROM users LIMIT 1")) {
                ...
                logger.info("[SchemaBootstrap] Core table 'users' does not exist.");
            }

            // Check 'activities' table (essential table verification check)
            try (ResultSet rs = stmt.executeQuery("SELECT 1 FROM activities LIMIT 1")) {
                essentialTableExists = true;
                logger.info("[SchemaBootstrap] Essential table 'activities' exists (verified via SELECT check).");
            } catch (SQLException e) {
                logger.info("[SchemaBootstrap] Essential table 'activities' does not exist.");
            }
        ...
        // If both tables exist, we can safely bypass all schema execution scripts
        if (usersTableExists && essentialTableExists) {
            logger.info("[SchemaBootstrap] Core tables ('users' and 'activities') are already initialized. Skipping all database schema bootstrap and migration tasks.");
            return;
        }

        logger.info("[SchemaBootstrap] Database is missing core tables. Initiating schema setup...");
        
        // 5. Run bootstrap.sql if users table is missing
        if (!usersTableExists) {
                    ...
                    for (String statement : statements) {
                        ...
                        if (!execSql.isEmpty()) {
                            try {
                                ...
                                logger.warn("[SchemaBootstrap] Statement failed during bootstrap: " + e.getMessage());
                            }
                        }
                    }
        ...
        // 6. Run other table creations if they do not exist
        try (Connection conn = DBConnection.getConnection();
             Statement stmt = conn.createStatement()) {
            ...
            // --- Drop constraints ---
            try { stmt.execute("ALTER TABLE hotel_bookings DROP FOREIGN KEY hotel_bookings_ibfk_2"); } catch (Exception e) {}
            try { stmt.execute("ALTER TABLE hotel_bookings DROP FOREIGN KEY hotel_bookings_ibfk_3"); } catch (Exception e) {}
            ...
            // --- Extended Columns for Destination Insights ---
            ...
            try { stmt.execute("ALTER TABLE posts ADD COLUMN hashtags VARCHAR(255) DEFAULT ''"); } catch (Exception e) {}
            try { stmt.execute("ALTER TABLE posts ADD COLUMN hidden BOOLEAN DEFAULT FALSE"); } catch (Exception e) {}

            // --- Extended Columns for site_content ---
            ...
            // --- Bookings Table Column Additions ---
            try { stmt.execute("ALTER TABLE destination_bookings ADD COLUMN travel_date DATE"); } catch (Exception e) {}
            try { stmt.execute("ALTER TABLE destination_bookings ADD COLUMN guests INT DEFAULT 1"); } catch (Exception e) {}

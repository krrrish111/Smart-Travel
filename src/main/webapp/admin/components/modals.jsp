<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<!-- Add/Edit Plan Modal -->
<div class="admin-modal-overlay" id="planModal">
    <div class="admin-modal" style="max-height: 90vh; overflow-y: auto;">
        <form id="planForm" action="${pageContext.request.contextPath}/plans" method="POST" enctype="multipart/form-data">
            <div class="admin-modal-header" style="position: sticky; top: 0; z-index: 100; background: var(--surface-light);">
                <h3 id="planModalTitle" style="margin: 0;">Add New Plan</h3>
            </div>
            
            <div class="admin-modal-body">
                <input type="hidden" id="planId" name="id">
                <input type="hidden" id="planAction" name="action" value="add">
                <div class="form-group">
                    <label class="form-label" for="planTitle">Plan Title</label>
                    <input type="text" class="form-control" id="planTitle" name="title" required placeholder="e.g. Majestic Alps Retreat">
                </div>
                <div class="grid" style="grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 24px;">
                    <div>
                        <label class="form-label" for="planDestination">Destination</label>
                        <select class="form-control" id="planDestination" name="destination_id" required style="padding-right:32px;">
                            <!-- Populated by JS -->
                        </select>
                    </div>
                    <div>
                        <label class="form-label" for="planCategory">Category</label>
                        <select class="form-control" id="planCategory" name="category" required style="padding-right:32px;">
                            <option value="Luxury">Luxury</option>
                            <option value="Adventure">Adventure</option>
                            <option value="Cultural">Cultural</option>
                            <option value="Family">Family</option>
                        </select>
                    </div>
                </div>
                <div class="grid" style="grid-template-columns: 1fr 1fr 1fr; gap: 16px; margin-bottom: 24px;">
                    <div>
                        <label class="form-label" for="planPrice">Price</label>
                        <input type="number" class="form-control" id="planPrice" name="price" required placeholder="1499">
                    </div>
                    <div>
                        <label class="form-label" for="planDays">Days</label>
                        <input type="number" class="form-control" id="planDays" name="days" required placeholder="7">
                    </div>
                    <div>
                        <label class="form-label" for="planNights">Nights</label>
                        <input type="number" class="form-control" id="planNights" name="nights" required placeholder="6">
                    </div>
                </div>
                <div class="form-group">
                    <label class="form-label" for="planImage">Plan Image</label>
                    <div style="border:2px dashed var(--color-border); border-radius:8px; padding:20px; text-align:center; position:relative; overflow:hidden; background:var(--surface-light); transition:border-color 0.2s;" id="planImageUploadBox">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="color:var(--text-muted); margin-bottom:8px; display:inline-block;"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                        <div style="font-size:0.9rem; color:var(--text-muted); margin-bottom:8px;">Upload image or paste URL below</div>
                        <input type="text" class="form-control" id="planImage" name="image" placeholder="Paste an image URL..." style="position:relative; z-index:10; margin-bottom: 10px;" autocomplete="off" oninput="previewImg('planImage','planImagePreview')">
                        <div style="font-size:0.8rem; color:var(--text-muted); margin-bottom:5px;">— OR —</div>
                        <input type="file" id="planImageFile" name="planImageFile" accept="image/*" class="form-control" style="font-size: 0.8rem;">
                    </div>
                    <img id="planImagePreview" class="image-preview-box" src="" alt="Preview">
                </div>
                <div class="form-group" style="padding-bottom: 20px;">
                    <label class="form-label" for="planDesc">Description / Highlight tags</label>
                    <textarea class="form-control" id="planDesc" name="desc" required placeholder="e.g. Mountains, Skiing, Luxury" rows="4" style="resize: vertical;"></textarea>
                </div>
            </div>
            
            <div class="admin-modal-footer" style="position: sticky; bottom: 0; z-index: 100; background: var(--surface-light);">
                <button type="button" class="btn btn-outline" onclick="closePlanModal()">Cancel</button>
                <button type="submit" class="btn btn-primary" id="savePlanBtn">Save Plan</button>
            </div>
        </form>
    </div>
</div>

<!-- Add/Edit Destination Modal -->
<div class="admin-modal-overlay" id="destModal">
    <div class="admin-modal" style="max-height: 90vh; overflow-y: auto;">
        <form id="destForm" action="${pageContext.request.contextPath}/destinations" method="POST" enctype="multipart/form-data">
            <div class="admin-modal-header" style="position: sticky; top: 0; z-index: 100; background: var(--surface-light);">
                <h3 id="destModalTitle" style="margin: 0;">Add Destination</h3>
            </div>
            
            <div class="admin-modal-body">
                <input type="hidden" id="destId" name="id">
                <input type="hidden" id="destAction" name="action" value="add">
                <div class="form-group">
                    <label class="form-label" for="destName">Destination Name</label>
                    <input type="text" class="form-control" id="destName" name="name" required placeholder="e.g. Santorini">
                </div>
                <div class="grid" style="grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 24px;">
                    <div>
                        <label class="form-label" for="destLocation">Location (State, City)</label>
                        <input type="text" class="form-control location-autocomplete" id="destLocation" name="location" required placeholder="e.g. Cyclades">
                    </div>
                    <div>
                        <label class="form-label" for="destCountry">Country</label>
                        <input autocomplete="country" type="text" class="form-control" id="destCountry" name="country" required placeholder="e.g. Greece">
                    </div>
                </div>
                
                <div class="grid" style="grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 24px;">
                    <div>
                        <label class="form-label" for="destCategory">Category</label>
                        <select class="form-control" id="destCategory" name="category" required style="padding-right:32px;">
                            <option value="Beach & Island">Beach & Island</option>
                            <option value="Mountain Retreat">Mountain Retreat</option>
                            <option value="City Escape">City Escape</option>
                            <option value="Heritage & Culture">Heritage & Culture</option>
                            <option value="Popular">Popular</option>
                        </select>
                    </div>
                    <div>
                        <label class="form-label" for="destPrice">Starting Price</label>
                        <input type="number" class="form-control" id="destPrice" name="price" placeholder="899">
                    </div>
                </div>

                <div class="grid" style="grid-template-columns: 1fr 1fr 1fr; gap: 16px; margin-bottom: 24px;">
                    <div>
                        <label class="form-label" for="destDurationDays">Days</label>
                        <input type="number" class="form-control" id="destDurationDays" name="duration_days" placeholder="5">
                    </div>
                    <div>
                        <label class="form-label" for="destDurationNights">Nights</label>
                        <input type="number" class="form-control" id="destDurationNights" name="duration_nights" placeholder="4">
                    </div>
                    <div>
                        <label class="form-label" for="destBestSeason">Best Season</label>
                        <input type="text" class="form-control" id="destBestSeason" name="best_season" placeholder="Summer">
                    </div>
                </div>

                <div class="grid" style="grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 24px;">
                    <div>
                        <label class="form-label" for="destLat">Latitude</label>
                        <input type="number" step="any" class="form-control" id="destLat" name="latitude" placeholder="36.3932">
                    </div>
                    <div>
                        <label class="form-label" for="destLng">Longitude</label>
                        <input type="number" step="any" class="form-control" id="destLng" name="longitude" placeholder="25.4615">
                    </div>
                </div>
                
                <div class="form-group">
                    <label class="form-label">Destination Hero Image</label>
                    <div style="border:2px dashed var(--color-border); border-radius:8px; padding:20px; text-align:center; position:relative; overflow:hidden; background:var(--surface-light); transition:border-color 0.2s;" id="destImageUploadBox">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="color:var(--text-muted); margin-bottom:8px; display:inline-block;"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                        <div style="font-size:0.9rem; color:var(--text-muted); margin-bottom:8px;">Upload image or paste URL below</div>
                        <input type="text" class="form-control" id="destImage" name="image" placeholder="Paste an image URL..." style="position:relative; z-index:10; margin-bottom: 10px;" autocomplete="off" oninput="previewImg('destImage','destImagePreview')">
                        <div style="font-size:0.8rem; color:var(--text-muted); margin-bottom:5px;">— OR —</div>
                        <input type="file" id="destImageFile" name="destImageFile" accept="image/*" class="form-control" style="font-size: 0.8rem;">
                    </div>
                    <img id="destImagePreview" class="image-preview-box" src="" alt="Preview">
                </div>

                <div class="form-group">
                    <label class="form-label">Gallery Images (Comma separated URLs for now)</label>
                    <input type="text" class="form-control" id="destGallery" name="gallery_images" placeholder="url1, url2...">
                    <div id="destGalleryPreview" style="display:flex; gap:8px; flex-wrap:wrap; margin-top:10px;"></div>
                </div>

                <div class="form-group">
                    <label class="form-label" for="destDesc">Description</label>
                    <textarea class="form-control" id="destDesc" name="desc" required placeholder="A beautiful island in Greece..." rows="4" style="resize: vertical;"></textarea>
                </div>
                
                <div class="form-group">
                    <label class="form-label" for="destHighlights">Highlights</label>
                    <textarea class="form-control" id="destHighlights" name="highlights" placeholder="Sunset views, Beaches..." rows="2" style="resize: vertical;"></textarea>
                </div>

                <div class="form-group" style="display:flex; gap:16px; flex-wrap:wrap;">
                    <label style="display:flex; align-items:center; gap:8px;"><input type="checkbox" id="destHasUnesco" name="has_unesco" value="true"> UNESCO Heritage</label>
                    <label style="display:flex; align-items:center; gap:8px;"><input type="checkbox" id="destIsTrending" name="is_trending" value="true"> Trending</label>
                    <label style="display:flex; align-items:center; gap:8px;"><input type="checkbox" id="destIsPopular" name="is_popular" value="true"> Popular</label>
                    <label style="display:flex; align-items:center; gap:8px;"><input type="checkbox" id="destIsFeatured" name="is_featured" value="true"> Featured</label>
                </div>
            </div>
            
            <div class="admin-modal-footer" style="position: sticky; bottom: 0; z-index: 100; background: var(--surface-light);">
                <button type="button" class="btn btn-outline" onclick="closeDestModal()">Cancel</button>
                <button type="submit" class="btn btn-primary" id="saveDestBtn">Save Destination</button>
            </div>
        </form>
    </div>
</div>

<!-- Add/Edit Activity Modal -->
<div class="admin-modal-overlay" id="activityModal">
    <div class="admin-modal">
        <h3 id="activityModalTitle" style="margin-bottom: 20px;">Add Activity</h3>
        <form id="activityForm">
            <input type="hidden" id="activityId" name="id">
            <input type="hidden" id="activityAction" name="action" value="add">
            
            <div class="form-group">
                <label class="form-label">Activity Name</label>
                <input type="text" class="form-control" id="activityName" name="name" required placeholder="e.g. Scuba Diving">
            </div>
            
            <div class="grid" style="grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 24px;">
                <div>
                    <label class="form-label">Destination</label>
                    <select class="form-control" id="activityDestId" name="destination_id" required style="padding-right:32px;">
                        <!-- Populated by JS -->
                    </select>
                </div>
                <div>
                    <label class="form-label">Price</label>
                    <input type="number" step="0.01" class="form-control" id="activityPrice" name="price" required placeholder="1200">
                </div>
            </div>

            <div class="form-group">
                <label class="form-label">Activity Image URL</label>
                <input type="text" class="form-control" id="activityImage" name="image_url" required placeholder="Paste an image URL..." oninput="previewImg('activityImage','activityImagePreview')">
                <img id="activityImagePreview" src="" alt="Preview" style="display:none; margin-top:10px; width:100%; height:120px; object-fit:cover; border-radius:8px; border:1px solid var(--color-border);">
            </div>
            
            <!-- These are usually hidden as they are updated by user reviews, but for admin edit we show them -->
            <div class="grid" id="activityExtraInfo" style="grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 24px; display:none;">
                <div>
                    <label class="form-label">Rating</label>
                    <input type="number" step="0.1" class="form-control" id="activityRating" name="rating" placeholder="4.5">
                </div>
                <div>
                    <label class="form-label">Reviews Count</label>
                    <input type="number" class="form-control" id="activityReviewsCount" name="reviews_count" placeholder="10">
                </div>
            </div>

            <div class="flex justify-end" style="gap: 12px; margin-top: 30px;">
                <button type="button" class="btn btn-outline" onclick="closeActivityModal()">Cancel</button>
                <button type="submit" class="btn btn-primary">Save Activity</button>
            </div>
        </form>
    </div>
</div>

<!-- Edit Content Modal -->
<div class="admin-modal-overlay" id="contentModal">
    <div class="admin-modal" style="max-height: 90vh; overflow-y: auto;">
        <div class="admin-modal-header" style="position: sticky; top: 0; z-index: 100; background: var(--surface-light);">
            <h3 id="contentModalTitle" style="margin: 0;">Edit Content</h3>
        </div>
        <form id="contentForm">
            <div class="admin-modal-body">
                <input type="hidden" id="contentId">

                <!-- Section Type (read-only display) -->
                <div class="form-group">
                    <label class="form-label">Section Type</label>
                    <input type="text" class="form-control" id="contentSectionType" readonly
                           style="background: var(--color-surface); cursor: default; font-size:0.8rem; color: var(--text-muted);">
                </div>

                <!-- Title -->
                <div class="form-group">
                    <label class="form-label">Title / Heading</label>
                    <input type="text" class="form-control" id="contentTitle" placeholder="e.g. Experience Voyastra">
                </div>

                <!-- Subtitle -->
                <div class="form-group">
                    <label class="form-label">Subtitle / Tagline</label>
                    <input type="text" class="form-control" id="contentSubtitle" placeholder="e.g. Discover the world with us">
                </div>

                <!-- Body Text -->
                <div class="form-group">
                    <label class="form-label">Body Text / Description</label>
                    <textarea class="form-control" id="contentBodyText" rows="4" style="resize: vertical;"
                              placeholder="Longer description text..."></textarea>
                </div>

                <!-- Image URL -->
                <div class="form-group">
                    <label class="form-label">Image URL</label>
                    <input type="url" class="form-control" id="contentImageUrl"
                           placeholder="https://example.com/image.jpg"
                           oninput="previewImg('contentImageUrl','contentImagePreview')">
                    <img id="contentImagePreview" src="" alt="Preview"
                         style="display:none; margin-top:10px; width:100%; height:120px; object-fit:cover; border-radius:8px; border:1px solid var(--color-border);">
                </div>

                <!-- Button Text + Link -->
                <div class="grid" style="grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 24px;">
                    <div>
                        <label class="form-label">Button Text</label>
                        <input type="text" class="form-control" id="contentButtonText" placeholder="e.g. Explore Now">
                    </div>
                    <div>
                        <label class="form-label">Button Link</label>
                        <input type="text" class="form-control" id="contentButtonLink" placeholder="e.g. /destinations">
                    </div>
                </div>

                <!-- Promo Code (shown only for promotion sections) -->
                <div class="form-group" id="contentPromoGroup">
                    <label class="form-label">Promo Code</label>
                    <input type="text" class="form-control" id="contentPromoCode" placeholder="e.g. SUMMER20"
                           style="font-family: monospace; letter-spacing: 1px;">
                </div>

                <!-- Active Toggle -->
                <div class="form-group" style="display:flex; align-items:center; gap:12px; margin-top:8px;">
                    <input type="checkbox" id="contentIsActive" style="width:18px; height:18px; cursor:pointer; accent-color: var(--color-primary);">
                    <label class="form-label" for="contentIsActive" style="margin:0; cursor:pointer;">Active (visible on site)</label>
                </div>
            </div>

            <div class="admin-modal-footer" style="position: sticky; bottom: 0; z-index: 100; background: var(--surface-light);">
                <button type="button" class="btn btn-outline" onclick="closeContentModal()">Cancel</button>
                <button type="submit" class="btn btn-primary" id="saveContentBtn">
                    <span id="saveContentBtnText">Save Content</span>
                </button>
            </div>
        </form>
    </div>
</div>

<!-- Custom Confirm Modal -->
<div class="admin-modal-overlay" id="confirmModal">
    <div class="admin-modal" style="text-align: center; max-width: 400px; border-top: 4px solid #ef4444;">
        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2" style="margin: 0 auto 16px auto;"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
        <h3 id="confirmTitle" style="margin-bottom: 12px; font-family: 'Poppins', 'Inter', 'Roboto', 'Arial', sans-serif;">Are you sure?</h3>
        <p id="confirmMessage" class="text-muted" style="margin-bottom: 24px;">This action cannot be undone.</p>
        <div class="flex justify-center" style="gap: 12px;">
            <button class="btn btn-outline" onclick="closeConfirmModal()">Cancel</button>
            <button class="btn btn-primary" id="confirmActionBtn" style="background: #ef4444; border-color: #ef4444;">Yes, Delete</button>
        </div>
    </div>
</div>

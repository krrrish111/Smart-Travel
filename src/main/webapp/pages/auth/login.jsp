...
<head>
    ...
    <!-- Removed Google Identity Services for standard OAuth flow -->
    <!-- Expose Java session to JS (prevents redirect loop) -->
    <script>
        (function(){
        ...
        // If already logged in server-side, skip login page immediately
        if (window.javaSession.userId && window.javaSession.userId.length > 0) {
            if (window.javaSession.role === 'admin') {
                window.location.replace(window.CONTEXT_PATH + '/admin');
            } else {
                window.location.replace(window.CONTEXT_PATH + '/user-home');
            }
        }
    </script>
    ...
    <div class="login-form-panel">
        <div class="login-form-wrap">
            ...
            <c:if test="${not empty errorMsg}">
                <div id="loginError" style="display:flex; align-items:center; gap:8px; padding:10px 14px; background:rgba(239,68,68,0.1); border:1px solid rgba(239,68,68,0.3); border-radius:8px; color:#ef4444; font-size:0.875rem; margin-bottom:16px;">
            ...
            <div id="googleAuthSection" style="margin-bottom:20px; display:flex; flex-direction:column; gap:10px;">
                <a href="${pageContext.request.contextPath}/google-login" style="display:flex; align-items:center; justify-content:center; gap:10px; width:100%; padding:14px; background:#ffffff; color:#0f0b08; border:1.5px solid rgba(15,11,8,0.15); border-radius:10px; font-family:'Poppins', sans-serif; font-size:0.9rem; font-weight:600; text-decoration:none; cursor:pointer; transition:background 0.2s, box-shadow 0.2s;">
            ...
            <div style="display:flex; align-items:center; gap:12px; margin-bottom:20px;">
                ...
                <span style="font-size:0.75rem; color:var(--text-muted); white-space:nowrap;">or continue with email</span>
                <div style="flex:1; height:1px; background:var(--color-border);"></div>
            </div>

            <!-- ── Email / Password Form ── -->
            <form action="${pageContext.request.contextPath}/login" method="POST" id="loginForm" novalidate data-vx>
                ...
                <div class="login-field">
                    ...
                    <input type="password" id="loginPassword" name="password" placeholder="Enter your password" autocomplete="current-password"
                           data-v-required data-v-label="Password">
            ...
            <div style="display:flex; flex-direction:column; align-items:center; gap:12px; margin-top:24px;">
                <a href="${pageContext.request.contextPath}/register" class="login-back" style="margin-top:0;">Don't have an account? Sign up</a>
...
<script>
    ...
    document.getElementById('loginForm').addEventListener('submit', function (e) {
        ...
        .then(function(data) {
            if (data.success) {
                ...
                setTimeout(function() {
                    window.location.href = data.redirect || (window.CONTEXT_PATH + '/user-home');
                }, 800);
            } else {
    ...
    (function() {
        ...
        if (params.get('logout') === 'true') {
            localStorage.removeItem('voyastra_user');
            setTimeout(function() {
                if (window.VoyastraToast) VoyastraToast.show('You have been logged out.', 'info');
            }, 200);
        }
    })();

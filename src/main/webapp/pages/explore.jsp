...
<main style="padding-top: 100px; padding-bottom: 80px; overflow-x: hidden;">
    ...
    <c:if test="${not empty searchQuery}">
        <div class="slide-up mb-8">
            <div class="flex items-center gap-3 mb-5">
                ...
                <a href="${pageContext.request.contextPath}/explore" class="btn btn-secondary" style="margin-left:auto; font-size:0.8rem; padding:6px 14px;">âœ• Clear</a>
            </div>
            ...
            <c:if test="${not empty destResults}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-8">
                    <c:forEach var="r" items="${destResults}">
                        <a href="${pageContext.request.contextPath}/destination?id=${r.id}" class="glass-panel" style="overflow:hidden; display:block; border-radius:var(--border-radius-lg);">
            ...
            <c:if test="${not empty planResults}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-8">
                    <c:forEach var="r" items="${planResults}">
                        <a href="${pageContext.request.contextPath}/planner?location=${r.title}" class="glass-panel" style="overflow:hidden; display:block; border-radius:var(--border-radius-lg);">
                            <div style="height:180px; overflow:hidden; position:relative;">
            ...
            <c:if test="${empty planResults and empty destResults}">
                <div class="vx-empty-state slide-up">
                    ...
                    <a href="${pageContext.request.contextPath}/explore" class="btn btn-primary" style="padding: 10px 24px;">Browse All Destinations</a>
                </div>
            </c:if>
    ...
    <div id="destinationsGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 slide-up delay-1 mb-12"
        ...
        <c:forEach var="dest" items="${destinations}">
            <a href="${pageContext.request.contextPath}/destination?id=${dest.id}" class="glass-panel" style="overflow: hidden; display: block; border-radius: var(--border-radius-lg); padding: 0;">
                <div style="height: 180px; overflow: hidden; position: relative;">
                    <img src="${not empty dest.imageUrl ? dest.imageUrl : 'https://images.unsplash.com/photo-1542332213-31f87348057f?auto=format&fit=crop&w=600&q=80'}" 
    ...
    <div id="plansGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 slide-up delay-2"
        ...
        <c:forEach var="plan" items="${plans}">
            <!-- Card -->
            <a href="${pageContext.request.contextPath}/planner?location=${plan.title}" class="glass-panel" style="overflow: hidden; display: block; border-radius: var(--border-radius-lg);">

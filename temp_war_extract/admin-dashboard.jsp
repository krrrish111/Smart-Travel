...
<%-- Security is now handled centrally by SecurityFilter --%>
...
document.addEventListener('DOMContentLoaded', () => {
    if (typeof VoyastraAuth === 'undefined') {
        document.getElementById('adminAccessDenied').style.display = 'flex';
        return;
    }
    VoyastraAuth.requireAuth(null, () => {
        const session = VoyastraAuth.getSession();
        const ADMIN_EMAILS = ['admin@voyastra.com']; 
        ...
        setTimeout(() => showToast('Welcome back, Server Admin.', 'success'), 1000);
        initAdminDashboard();
    });
});
